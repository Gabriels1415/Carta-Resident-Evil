...
function CrearCarta() {
  ...
  // Cada vez que se escribe en un cuadro de texto, lo guardamos en el estado
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    const { name, value } = e.target;

    // Si el cambio es el tipo o el arma favorita
    if (name === 'tipo' || name === 'armaFavorita') {
      ...
      // Para los demás campos de la carta
      setFormData({
        ...formData,
  ...
  // Al darle clic al botón de "Registrar Carta"
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setCargando(true);
    try {
      // Preparamos los datos finales del formulario
      const datosParaAPI = {
        ...formData,
  ...
  return (
    <div className="max-w-2xl mx-auto bg-black/80 p-8 rounded-xl border border-red-900/50 shadow-[0_0_30px_rgba(255,0,0,0.15)] backdrop-blur-md text-white mt-10">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {/* Cajas para Ataque, Defensa y Vida */}
        ...
        {/* Botón para enviar el formulario cargado */}
        <button
          type="submit"
          disabled={cargando}
